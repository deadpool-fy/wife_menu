...
这是一个基于Node.js的小红书菜谱数据爬虫服务，用于自动爬取菜谱数据并存储到MySQL数据库中，为微信小程序提供真实的菜谱数据。
...
```bash
mysql -h localhost -u root -p wife_menu < scripts/init-database.sql
```
